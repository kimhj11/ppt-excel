...
st.title("✍️ 온라인 평가문항 양식으로 자동 변환하기")
...
# 👉 텍스트 직접 입력으로 문항 추가
with st.expander("📝 텍스트로 직접 문항 입력하기 (선택)", expanded=False):
    manual_text = st.text_area("아래에 문항 텍스트를 입력하세요.", height=300, key="manual_input")
    if st.button("📤 문항 추출하기", key="extract_manual"):
...
# 기존 세션 수동 입력 반영
data_rows.extend(st.session_state.manual_rows)
